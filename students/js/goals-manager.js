...
async function calculateGoalProgress(sb, userId, goal, topicProgress, quizResults) {
    ...
    switch (goalType) {
        ...
        case 'subject_mastery': {
            ...
            // This combines topic progress (sections viewed) with quiz performance
            if (!subject) return 0;
            
            const subjectProgress = topicProgress.filter(tp => 
                tp.subject?.toLowerCase() === subject.toLowerCase()
            );
            ...
            // Calculate topic progress (sections viewed)
            let progressFromTopics = 0;
            if (subjectProgress.length > 0) {
                progressFromTopics = subjectProgress.reduce((sum, tp) => 
            ...
            if (subjectQuizzes.length > 0) {
                // Average quiz percentage for this subject
                progressFromQuizzes = subjectQuizzes.reduce((sum, q) => 
                    sum + (q.percentage || 0), 0
                ) / subjectQuizzes.length;
            }
            
            // Combine both metrics (50% topic progress, 50% quiz performance)
            // If no quizzes taken, use only topic progress
            // If no topics viewed, use only quiz performance
            if (subjectProgress.length === 0 && subjectQuizzes.length === 0) {
                return 0;
            }
            
            let masteryScore = 0;
            if (subjectProgress.length > 0 && subjectQuizzes.length > 0) {
                // Both available: weight equally
                masteryScore = (progressFromTopics * 0.5) + (progressFromQuizzes * 0.5);
            } else if (subjectProgress.length > 0) {
                // Only topic progress available
                masteryScore = progressFromTopics;
            } else {
                // Only quiz results available
                masteryScore = progressFromQuizzes;
            }
            
            console.log(`📊 subject_mastery for ${subject}: topics=${progressFromTopics.toFixed(1)}%, quizzes=${progressFromQuizzes.toFixed(1)}%, final=${masteryScore.toFixed(1)}%`);
