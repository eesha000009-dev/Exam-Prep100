...
async function saveGoals() {
    ...
    // Validate we have a primary goal selected
    if (!sel.primaryGoal) {
        showToast('Please select a primary goal', 'warning');
        return false;
    }
    ...
    if (!window.GoalsManager) {
        console.error('❌ GoalsManager not found! Waiting for it to load...');
        showToast('Loading goals system...', 'info');
        
        // Wait a bit and try again
        await new Promise(resolve => setTimeout(resolve, 2000));
        
        if (!window.GoalsManager) {
            console.error('❌ GoalsManager still not found after waiting');
            showToast('Error: Goals system not loaded. Please refresh the page.', 'error');
            return false;
        }
    }
    ...
    try {
        ...
        console.log('🚀 Calling GoalsManager.createMultipleGoals...');
        ...
        if (result.success) {
            ...
            // Debug: Show the actual error in console and toast
            console.log('🔍 Error type detected:', errorMsg);
            
            if (errorMsg.includes('not_authenticated')) {
                showToast('⚠️ Please log in to save your goals. You may need to sign in first.', 'warning');
                console.log('💡 TIP: User is not authenticated. Check if Supabase auth session exists.');
            } else if (errorMsg.includes('supabase_not_initialized')) {
                showToast('❌ Connection error. Please check your internet and refresh the page.', 'error');
                console.log('💡 TIP: Supabase client failed to initialize. Check console for bootstrap errors.');
            } else if (errorMsg.includes('policy') || errorMsg.includes('permission') || errorMsg.includes('42501')) {
                showToast('⚠️ Permission error. The database may need RLS policies configured.', 'warning');
            } else if (errorMsg.includes('does not exist') || errorMsg.includes('42P01')) {
                showToast('⚠️ Database table not found. Please run the SQL setup script in Supabase.', 'warning');
            } else {
...
async function initializeGoalsCheck() {
    console.log('🔍 GoalsModal: Checking for existing goals...');
    
    // Check if GoalsManager is available
    if (!window.GoalsManager) {
        console.warn('⚠️ GoalsModal: GoalsManager not loaded yet, waiting...');
        // Wait and retry
        setTimeout(initializeGoalsCheck, 500);
        return;
    }
    
    // Also wait for Supabase to be ready
    if (!window.supabaseClient) {
        console.log('⏳ GoalsModal: Waiting for Supabase client...');
        ...
        await new Promise((resolve) => {
            const checkInterval = setInterval(() => {
                if (window.supabaseClient || Date.now() - startTime > maxWait) {
        ...
        if (!window.supabaseClient) {
            console.warn('⚠️ GoalsModal: Supabase client not available after waiting');
            return;
        }
    }
    
    console.log('✅ GoalsModal: All dependencies ready');
    
    try {
        const result = await window.GoalsManager.checkUserGoals();
        
        console.log('📊 GoalsModal: Goals check result:', result);
        
        if (result.error === 'not_authenticated') {
            console.log('ℹ️ GoalsModal: User not authenticated, skipping goals check');
            return;
        }
        
        if (result.error === 'table_not_found') {
            console.warn('⚠️ GoalsModal: user_goals table not found. Run the SQL setup script.');
            showToast('Database not set up. Please run the SQL setup script in Supabase.', 'warning');
            return;
        }
        
        if (result.error === 'permission_denied') {
            console.warn('⚠️ GoalsModal: Permission denied. Check RLS policies.');
            showToast('Permission error. Please check database policies.', 'warning');
            return;
        }
        
        if (result.error === 'supabase_not_initialized') {
            console.warn('⚠️ GoalsModal: Supabase not initialized');
            showToast('Connection error. Please refresh the page.', 'warning');
            return;
        }
        
        if (result.firstTime || !result.hasGoals) {
            console.log('👋 GoalsModal: First time user - showing goals modal');
            setTimeout(() => {
                openGoalsModal();
            }, 1500);
        } else {
            console.log('✅ GoalsModal: User has existing goals:', result.goals.length);
        }
        
    } catch (err) {
        console.error('❌ GoalsModal: Error checking goals:', err);
    }
}
...
window.GoalsModal = {
    ...
    checkGoals: initializeGoalsCheck,
